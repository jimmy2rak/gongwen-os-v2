...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ success: true, data, providers: AI_PROVIDERS });
  } catch (error) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await client.execute({
      sql: "INSERT INTO api_keys (id, user_id, provider, encrypted, iv, models, default_model, is_active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      args: [id, user.id, provider, encrypted, iv, JSON.stringify(modelList), modelList[0], 1, now, now],
    });
...
// ─── PUT 更新（models / isActive / defaultModel / 可选重填 key）──
export async function PUT(req: NextRequest) {
  ...
  try {
    const body = await req.json();
    const { id, apiKey, models, isActive, defaultModel } = body;
    if (!id) {
